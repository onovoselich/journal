...
<div class="main-part">
    ...
    <table>
        ...
        <form action="/secretary/subjects/alter_subject" method="post">
                ...
                <td><input type="text" name="name" required="required"  size="30" value="${subj.getName()}"/></td>
                ...
                <td><%--<input type="text" required="required"  name="hours" size="5"  class="integer"  value="${subj.getHours()}"/>--%>
                    ...
                    <c:forEach var="i" begin="1" end="8">
                        <td >
                        <input <c:if test="${subj.getSums().keySet().contains(i)}">checked </c:if> required="required"   type="checkbox" name="sum" id="sum${i}_${subj.getId()}" value="${i}">
                        <label  for="sum${i}_${subj.getId()}"> <%=Number.arabic2roman((Integer)pageContext.getAttribute("i"))%> </label>
                        </td>
                    </c:forEach>
                    </tr>
                    <tr>
                        <c:forEach var="i" begin="1" end="8">
                            <td >
                                <input class="integer"  size="1" type="text" name="sum${i}"  value="${subj.getSums().get(i)}">
