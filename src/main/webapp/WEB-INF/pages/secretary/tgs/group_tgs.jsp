                    ...
                    <tr>
                        <td><select name="subj_id" required="required">
                            ...
                            <c:forEach items="${subj_list}" var="subj">
                                <option value="${subj.getId()}"
                                        <c:if test="${subj.getId() == tuple.value.getX().getId()}">selected</c:if>>${subj}</option>
                            </c:forEach>
                        </select></td>
                        <td><select name="teac_id" required="required">
                            <option value="" selected></option>
                            <c:forEach items="${teac_list}" var="teac">
                                <option value="${teac.getId()}"
                                        <c:if test="${teac.getId() == tuple.value.getY().getId()}">selected</c:if>>${teac}</option>
                            </c:forEach>
                        </select></td>
