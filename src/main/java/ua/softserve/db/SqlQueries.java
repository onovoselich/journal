...
//    public static final String GET_ALL_STUDENTS = "select `Id`, `Name`,`Surname`,`Patronimic`,`gradebook` from `students`";
    ...
    public static final String UPDATE_SUBJECT = "UPDATE `subjects` SET `Name` = ?,`Shortcut` = ?,`ECTSCredits`=?,`Sums`=?,`ControlForm`=?,`specId`=?  WHERE `subjects`.`Id` = ?;";
    public static final String INSERT_SUBJECT = "INSERT INTO `subjects` ( `Name`,`Shortcut`,`ECTSCredits`,`Sums`,`ControlForm`,`specId`)  values (?,?,?,?,?,?);";
    ...
    public static final String VIDRAH_STUDENT = "UPDATE `students` SET `Status` =  'vidrah' WHERE `id` = ?;";
    public static final String REESTABLISH_STUDENT = "UPDATE `students` SET `Status` =  null WHERE `id` = ?;";
